...
class RankState:
    ...
    def set_health_status(self, status: HealthStatus, reason: Optional[str] = None) -> None:
...
class StateManager:
    ...
    def get_active_ranks(self) -> list:
        ...
        return [rank for rank, state in self._states.items() if state.mode == RankMode.ACTIVE]
