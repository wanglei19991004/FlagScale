...
class HeartbeatSender:
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._heartbeat_loop, name=f"HeartbeatSender-rank{self.rank}", daemon=True
        )
        self._thread.start()
        logger.info(
    ...
    def _send_heartbeat(self) -> None:
        ...
        with self._lock:
            record = HeartbeatRecord(
                ...
                metrics={"rank": self.rank, "pid": os.getpid()},
            )
...
class HeartbeatMonitor:
            ...
            for rank in range(self.world_size):
                ...
                if age > timeout:
                    if not self._timeout_status.get(rank, False):
                        ...
                        self._timeout_counts[rank] = self._timeout_counts.get(rank, 0) + 1
    ...
    def trigger_restart_for_rank(self, rank: int, reason: str = None) -> None:
        ...
        raise RankShouldRestart(reason=restart_reason, rank=rank, fault_type="manual")
        ...
        with self._lock:
            ...
            for rank in range(self.world_size):
                ...
                if record:
                    ...
                    ranks_status[rank] = {"last_heartbeat": None, "status": "no_heartbeat_received"}

            return {
                "world_size": self.world_size,
                "ranks": ranks_status,
                "total_timeouts": sum(self._timeout_counts.values()),
                "currently_timed_out": sum(1 for v in self._timeout_status.values() if v),
            }
    ...
    def get_healthy_ranks(self) -> List[int]:
        ...
        with self._lock:
            return [
                rank
                for rank in range(self.world_size)
                if not self._timeout_status.get(rank, False) and rank in self._last_heartbeats
            ]

    def get_timed_out_ranks(self) -> List[int]:
        """Get list of ranks that are currently timed out."""
        with self._lock:
            return [rank for rank, timed_out in self._timeout_status.items() if timed_out]
...
class RankMonitorClient:
    ...
    def __init__(self, rank: int, world_size: int, config: HeartbeatConfig = None):
    ...
    def init_workload_monitoring(
        cls, rank: int = None, world_size: int = None, config: HeartbeatConfig = None
    ) -> "RankMonitorClient":
        ...
        logger.info(f"Workload monitoring initialized for rank {rank}/{world_size}")
        return cls._instance
    ...
    def send_heartbeat(self, iteration: int = None, phase: HeartbeatPhase = None) -> None:
...
def calculate_and_set_hb_timeouts(
    config: HeartbeatConfig, observed_intervals: List[float], multiplier: float = 3.0
) -> HeartbeatConfig:
