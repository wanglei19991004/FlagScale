...
from .exception import MonitorError, RankShouldRestart, StoreError
...
from .state import FrozenRankState, HealthStatus, RankMode, RankState, StateManager
...
class InProcessMonitor:
    ...
    def __init__(
        ...
        self._heartbeat_sender = HeartbeatSender(
            rank=self.rank, config=self.heartbeat_config, on_heartbeat=self._on_local_heartbeat
        )
    ...
    def _default_health_checks(self) -> List[HealthCheck]:
        ...
        return [CudaHealthCheck(), NvmlHealthCheck(), FaultCounter(max_rank_faults=5)]
    ...
    def start(self) -> None:
        ...
        self._health_check_thread = threading.Thread(
            target=self._health_check_loop, name=f"HealthCheckThread-rank{self.rank}", daemon=True
        )
        self._health_check_thread.start()

        logger.info(f"InProcessMonitor started for rank {self.rank}/{self.world_size}")

        # Record start event
        self._record_event(
            MonitorEvent.STATUS_CHANGE, {"status": "started", "mode": RankMode.ACTIVE.value}
        )
    ...
    def stop(self) -> None:
        ...
        # Record stop event
        self._record_event(
            MonitorEvent.STATUS_CHANGE, {"status": "stopped", "mode": RankMode.TERMINATED.value}
        )
    ...
    def ping(
        self, iteration: int = None, phase: HeartbeatPhase = None, metrics: Dict[str, Any] = None
    ) -> None:
    ...
    def enter_checkpoint_phase(self) -> None:
        ...
        self._record_event(MonitorEvent.STATUS_CHANGE, {"phase": HeartbeatPhase.CHECKPOINT.value})
    ...
    def exit_checkpoint_phase(self) -> None:
        ...
        self._record_event(MonitorEvent.STATUS_CHANGE, {"phase": HeartbeatPhase.TRAINING.value})
    ...
    def record_fault(self, reason: str, error: Exception = None) -> int:
        ...
        self._record_event(
            MonitorEvent.FAULT,
            {"reason": reason, "error": str(error) if error else None, "fault_count": fault_count},
            severity="warning",
        )

        logger.warning(
            f"Fault recorded for rank {self.rank}: {reason} " f"(total faults: {fault_count})"
        )
    ...
    def trigger_restart(
        ...
        self._record_event(
            MonitorEvent.FAULT,
            {"trigger": fault_type, "reason": reason, "restart_requested": True},
            severity="error",
        )
    ...
    def _on_local_heartbeat(self, record: HeartbeatRecord) -> None:
        ...
        self._record_event(
            ...
            {"iteration": record.iteration, "phase": record.phase.value},
            severity="debug",
        )
    ...
    def _run_health_checks(self) -> None:
        ...
        if all_healthy:
            ...
            if self.enable_restart_on_failure and failed_checks:
                ...
                # Raise restart exception
                raise RankShouldRestart(reason=reason, rank=self.rank, fault_type="health_check")

    def _record_event(
        self, event_type: MonitorEvent, data: Dict[str, Any], severity: str = "info"
    ) -> None:
        """Record a monitoring event."""
        event = MonitorEventRecord(
            event_type=event_type, rank=self.rank, data=data, severity=severity
        )
    ...
    def _write_final_log(self) -> None:
        ...
        try:
            log_file = os.path.join(self.log_dir, f"rank_{self.rank}_monitor_log.json")
...
class DistributedMonitor:
    ...
    def __init__(
        ...
        self._heartbeat_monitor = HeartbeatMonitor(
            world_size=world_size, config=heartbeat_config, on_timeout=on_timeout
        )
    ...
    def start(self) -> None:
        ...
        self._monitor_thread = threading.Thread(
            target=self._monitor_loop, name="DistributedMonitor", daemon=True
        )
