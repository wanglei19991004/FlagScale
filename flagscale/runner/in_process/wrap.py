...
from .initialize import RestartConfig, RetryController, create_default_retry_controller
from .monitor import InProcessMonitor, MonitorEvent, MonitorEventRecord
from .progress_watchdog import ProgressWatchdog
from .rank_assignment import MaxActiveWorldSize, RankAssignmentCtx, SimpleRankAssignment
...
class Wrapper:
    ...
    def __init__(
        ...
        if self.config.enable_restart:
            self._retry_controller = create_default_retry_controller(
                max_restarts=self.config.max_restarts, min_world_size=self.config.min_world_size
            )
    ...
    def _on_event(self, event: MonitorEventRecord) -> None:
        ...
        if event.event_type == MonitorEvent.HEALTH_CHECK:
            ...
            if self.on_fault:
                self.on_fault(event.data.get("reason", ""), event.data.get("error"))
    ...
    def start(self) -> "Wrapper":
        ...
        enable_restart_on_failure = (
            self.config.enable_restart and self.config.restart_on_health_check_fail
        )
        ...
        if self._restart_coordinator is not None:
            if self._restart_coordinator.restart_requested(self._state.restart_attempt):
                ...
                raise RankShouldRestart(
                    reason=f"Peer restart: {reason}", rank=self.rank, fault_type="peer_restart"
                )
    ...
    def run_with_restart(self, fn: Callable, *args, **kwargs) -> Any:
        ...
        while True:
            ...
            assigned_ctx = RankAssignmentCtx(
                state=self._state, terminated_ranks=self._terminated_ranks.copy()
            )
            ...
            if self._state.mode == RankMode.INACTIVE:
                logger.info(
                    f"Rank {self.rank}: Assigned INACTIVE (spare). Waiting for next restart..."
                )
                try:
                    ...
                    if self._restart_coordinator:
                        ...
                        while True:
                            if self._restart_coordinator.restart_requested(
            ...
            try:
                ...
                if self.config.restart_on_exception:
                    logger.warning(f"Rank {self.rank}: Exception caught, triggering restart: {e}")
                    last_error = e
    ...
    def _handle_restart(self, reason: str, error: Exception = None) -> None:
        ...
        if self._restart_coordinator:
            ...
            if info:
                ...
                if failed_initial_rank >= 0:
                    logger.warning(
                        f"Rank {self.rank}: Detected failure of initial_rank {failed_initial_rank}"
                    )
                    self._terminated_ranks.add(failed_initial_rank)
                else:
                    logger.warning(
                        f"Rank {self.rank}: Could not determine initial_rank of failed rank {failed_rank}"
                    )
        # Record the fault
        ...
        if self._restart_coordinator is not None:
            ...
            if not barrier_ok:
                logger.warning(f"Rank {self.rank}: abort_done barrier timeout, proceeding anyway")
...
class CallWrapper:
    ...
    def __init__(self, wrapper: Wrapper, fn: Callable, args: tuple = (), kwargs: dict = None):
...
def init_in_process_monitoring(config: Dict[str, Any] = None, **kwargs) -> Wrapper:
