    ...
    def check(self, state: FrozenRankState) -> HealthCheckResult:
        ...
        if local_rank >= device_count:
            return HealthCheckResult(
                ...
                metrics={"local_rank": local_rank, "device_count": device_count},
            )
        ...
        try:
            ...
            return HealthCheckResult(
                ...
                metrics={"device": local_rank, "error": error_msg},
            )
    ...
    def check(self, state: FrozenRankState) -> HealthCheckResult:
        ...
        try:
            ...
            if mem_used_ratio > self.memory_threshold:
                issues.append(f"Memory usage {mem_used_ratio:.1%} > {self.memory_threshold:.1%}")
            if ecc_errors > 0:
                issues.append(f"ECC errors detected: {ecc_errors}")

            if issues:
                return HealthCheckResult(
                    healthy=False, check_name=self.name, reason="; ".join(issues), metrics=metrics
                )

            return HealthCheckResult(
                healthy=True, check_name=self.name, reason="GPU healthy", metrics=metrics
            )
...
class NetworkHealthCheck(HealthCheck):
    ...
    def __init__(self, interfaces: List[str] = None, name: str = "NetworkHealthCheck"):
    ...
    def check(self, state: FrozenRankState) -> HealthCheckResult:
        ...
        return HealthCheckResult(
            healthy=True, check_name=self.name, reason="Network interfaces healthy", metrics=metrics
        )
...
class FaultCounter(HealthCheck):
    ...
    def __init__(self, max_rank_faults: int = 3, name: str = "FaultCounter"):
...
class HealthCheckRunner:
    ...
    def _default_checks(self) -> List[HealthCheck]:
        ...
        return [CudaHealthCheck(), NvmlHealthCheck(), NetworkHealthCheck()]
    ...
    def run_all(self, state: RankState) -> List[HealthCheckResult]:
        ...
        for check in self.checks:
            ...
            # Log result
            if result.healthy:
                logger.debug(f"Health check {result.check_name} passed: {result.reason}")
            else:
                logger.warning(f"Health check {result.check_name} failed: {result.reason}")
